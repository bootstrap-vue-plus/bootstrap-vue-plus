...
import { getStyle } from '@bootstrap-vue-plus/utils'
import { rAF } from '@bootstrap-vue-plus/test-utils/tick'
import { BvMessage } from '..'
...
describe('Message on command', () => {
  ...
  describe('context inheritance', () => {
    it('should globally inherit context correctly', () => {
      expect(BvMessage._context).toBe(null)
      const testContext = {
        config: {
          globalProperties: {},
        },
      ...
      BvMessage.install?.(testContext as any)
      expect(BvMessage._context).not.toBe(null)
      expect(BvMessage._context).toBe(testContext._context)
      // clean up
      BvMessage._context = null
    })
  })
})
