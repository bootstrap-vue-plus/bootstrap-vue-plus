...
import { isArray, isNull, isPlainObject, isString, isUndefined } from './types'
...
export const stringifyQueryObj = (obj: object) => {
  if (!isPlainObject(obj)) {
    return ''
  }

  const query = keysOf(obj)
    .map((key) => {
...
export const parseQuery = (query: string) => {
  ...
  query.split('&').forEach((param) => {
    const parts: Array<string> = param.replace(RX_PLUS, ' ').split('=')
...
export const isLink = (props: HrefType) => !!(props.href || props.to)

export const isRouterLink = (tag: string) => !!(tag && !isTag(tag, 'a'))
...
export const computeTag = (
  { to, disabled, routerComponentName }: TagType,
  thisOrParent: any
) => {
  ...
  return (
    routerComponentName || (thisOrParent.$nuxt ? 'nuxt-link' : 'router-link')
  )
}
...
export const computeRel = ({ target, rel }: RelType = {}) =>
